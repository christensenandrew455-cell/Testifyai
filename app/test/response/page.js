...
export default function Response({ question, onAnswer, topic, currentIndex, totalQuestions }) {
  const [answer, setAnswer] = useState("");
  const router = useRouter();
  if (!question) return null;

  const handleCheck = () => onAnswer({ correct: null });
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", backgroundColor: "#f8fafc", padding: "40px 20px", fontFamily: "Segoe UI, Roboto, sans-serif", color: "#222" }}>
      ...
      {/* Answer Box */}
      <textarea value={answer} onChange={(e) => setAnswer(e.target.value)} placeholder="Type your answer here..." style={{ width: "100%", maxWidth: "600px", height: "120px", border: "2px solid rgba(0,0,0,0.1)", borderRadius: "12px", padding: "10px", fontSize: "1rem", fontFamily: "inherit", marginBottom: "24px", boxShadow: "0 2px 6px rgba(0,0,0,0.05)" }} />
