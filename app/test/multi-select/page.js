...
export default function MultiSelect({ question, onAnswer, topic, currentIndex, totalQuestions }) {
  ...
  const handleCheck = () => {
    ...
    const correct = correctSet.size === selectedSet.size && [...correctSet].every((val) => selectedSet.has(val));
    onAnswer({ correct });
  };
    ...
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", backgroundColor: "#f8fafc", padding: "40px 20px", fontFamily: "Segoe UI, Roboto, sans-serif", color: "#222" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "12px", width: "100%", maxWidth: "600px" }}>
        {question.answers?.map((ans, i) => (
          <button key={i} onClick={() => handleToggle(i)} style={{ display: "flex", alignItems: "center", padding: "12px 20px", borderRadius: "12px", border: selected.includes(i) ? "3px solid #1976d2" : "2px solid rgba(0,0,0,0.1)", backgroundColor: selected.includes(i) ? "rgba(25,118,210,0.1)" : "white", cursor: "pointer", fontWeight: 500 }}>
