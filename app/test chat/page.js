...
export default function TestChat() {
  ...
  const handleNext = () => {
    ...
    if (currentQuestion < questions.length - 1) {
      ...
      // Go to results page and pass score via URL
      window.location.href = `/results?score=${score + (selectedAnswer === questions[currentQuestion].correct ? 1 : 0)}&total=${questions.length}`;
    }
  };
  ...
  return (
    <div className="min-h-screen bg-orange-50 flex flex-col items-center justify-center text-gray-900 p-6">
      ...
      <p className="text-gray-700 mb-6">Question {currentQuestion + 1} of {questions.length}</p>
