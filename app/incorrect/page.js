...
function IncorrectContent() {
  ...
  try{const stored=sessionStorage.getItem("testData");if(stored){const parsed=JSON.parse(stored);questionObj=parsed.questions?.[index]||null;}}catch{}
  ...
  function mapToLetterText(answerValue,questionObj){
    ...
    const mapSingle=val=>{
      ...
      if(typeof val==="number"&&answers[val]!==undefined){const raw=answers[val];if(isTrueFalse)return String(raw);return `${String.fromCharCode(65+val)}. ${raw}`;}
  ...
  const handleContinue=()=>{if(!canContinue)return;sessionStorage.setItem("currentIndex",String(index+1));router.push("/test/controller");};

  return (
    <div onClick={handleContinue} style={{height:"100vh",width:"100vw",display:"flex",flexDirection:"column",justifyContent:"center",alignItems:"center",background:"linear-gradient(to right, #ff8a80, #e53935)",color:"white",textAlign:"center",fontFamily:"Segoe UI, Roboto, sans-serif",cursor:canContinue?"pointer":"default",padding:"20px",transition:"opacity 0.3s ease",opacity:canContinue?1:0.8}}>
...
export default function IncorrectPage() { return <Suspense fallback={null}><IncorrectContent/></Suspense>; }
